...
class SmartThingsEntity(Entity):
    ...
    async def execute_device_command(
        ...
        """Execute a command on the device and refresh status immediately."""
        kwargs = {}
        ...
        # Immediately fetch and update status after command
        from . import process_status
        status = process_status(
            await self.client.get_device_status(self.device.device.device_id)
        )
        # Update internal state and UI
        if self.component in status:
            for cap in self.capabilities:
                if cap in status[self.component]:
                    self._internal_state[cap] = status[self.component][cap]
            self._handle_update()
